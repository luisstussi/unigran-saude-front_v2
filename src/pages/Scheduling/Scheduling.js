...
import Header from "../../components/RepeatedElements/Header";
  ...
  const applyFiltersFunction = () => {
    ...
    SchedulingAPI.listSchedulings(
      ...
      `${dateToUse.getFullYear()}-${
        dateToUse.getMonth() + 1 <= 9
          ? `0${dateToUse.getMonth() + 1}`
  ...
  return (
    <div>
      ...
      <section className="sectionBackgroundScheduling">
        ...
        {showCreateScheduling === true ? (
          <CreateScheduling
            setShowCreateScheduling={setShowCreateScheduling}
            allAttendants={allAttendants}
            allOfClients={allOfClients}
          />
        ) : (
          ""
        )}
        ...
        <Header
          title={"Agendamentos"}
          btnTitle={"Criar Agendamento"}
          btnFunction={setShowCreateSchedulingFunction}
        />
